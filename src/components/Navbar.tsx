    ...
    <header className="sticky top-0 z-40 w-full border-b border-[#E4DFD3] bg-[#F5F3EC]/95 backdrop-blur-md transition-all shadow-xs pt-safe">
      <div className="max-w-7xl mx-auto px-3.5 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14 sm:h-16">
          ...
          {/* Action Button: Create Session (Responsive fix for mobile) */}
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#FAF8F5]/98 backdrop-blur-xl border-t border-[#E4DFD3] px-2 py-1.5 flex justify-around items-center shadow-lg pb-safe">
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive = activeTab === item.id;
          return (
