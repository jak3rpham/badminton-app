...
import { Users, Plus, Trash2, Phone, Sparkles, Search, Check, WalletCards } from 'lucide-react';
    ...
    <div className="space-y-6 pb-20 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-400 mt-1">
            Quản lý {members.length} thành viên thường xuyên và vãng lai của nhóm cầu lông
          </p>
        </div>
      ...
      {isAdding && (
        <form 
          ...
          className="p-5 sm:p-6 rounded-3xl bg-slate-900 border border-slate-800 shadow-xl space-y-4 animate-fadeIn"
        >
          <h3 className="text-sm font-bold text-white uppercase tracking-wider">Thông Tin Thành Viên Mới</h3>
          
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-semibold text-slate-400 mb-1">Họ và tên / Biệt danh *</label>
              <input
                type="text"
                required
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-400 mb-1">Số điện thoại (tùy chọn)</label>
              <input
                type="tel"
                placeholder="0912..."
                value={newPhone}
            ...
            <div className="flex items-center gap-2 pt-6">
              <label className="flex items-center gap-2 text-xs text-slate-300 cursor-pointer select-none">
                ...
                />
                <span>Thành viên cố định (đánh hàng tuần)</span>
              </label>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredMembers.map((member) => {
          ...
          return (
            <div
              key={member.id}
              className="rounded-3xl glass-card border border-slate-800 p-5 shadow-lg flex flex-col justify-between"
            >
              <div>
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <div
                      className="w-12 h-12 rounded-2xl flex items-center justify-center font-bold text-white text-base shadow-sm shrink-0 border border-white/10"
                      style={{ backgroundColor: member.avatarColor }}
                    >
                      {getInitials(member.name)}
                    </div>
                    <div>
                      <h3 className="font-bold text-white text-sm sm:text-base tracking-tight">{member.name}</h3>
                      {member.phone && (
                ...
                <div className="mt-4 flex items-center gap-2">
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${
                    ...
                    {member.isRegular ? 'Cố định' : 'Vãng lai'}
                  </span>
                  <span className="text-xs text-slate-400">
                    Đã tham gia {gamesJoinedCount} buổi
                  </span>
                </div>
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-slate-800/80 flex items-center justify-between">
                <div>
                  ...
                  {playerDebt && playerDebt.totalDebt > 0 ? (
                    ...
                    <span className="text-xs font-bold text-emerald-400">Không nợ tiền</span>
                  )}
                </div>
